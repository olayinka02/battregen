...
const WelcomePage = ({ welcomeTitle, paragraph, imageSrc, buttonOne,subheading, welcomecolor }) => {
  return (
    <div className="w-full flex justify-center bg-brandblack  md:pt-44 text-white">
         <div className="hero-section grid grid-cols-1 pb-3 pt-64 mt-0 lg:pb-16  md:pb-16 lg:mt-52   h-auto md:grid-cols-2  md:h-auto w-full md:w-4/5  md:pt-5 px-4 md:md-4" >
      ...
      <div className="flex space-x-4">
        {buttonOne && (
          <Link href={buttonOne.link}>
            <button className=" text-brandblack px-4 py-3 rounded-full text-xs bg-white">
              {buttonOne.text}
            </button>
          </Link>
        )}
